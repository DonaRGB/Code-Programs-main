...
import io
def generate_responses(prompt:str,temperature:float = 0.1):
    try:
...
**World Problems :** Real-world applications of mathematics
-Example : \"A train travels 300 miles in 4 hours. How fast was it going?\"""")
    ...
    with st.form(key = "math_form",clear_on_submit = True):
        ui = st.text_area("????? Enter your math problem here :",height = 100,placeholder = "Example : Solve x² + 5x + 6 = 0 or Find the integral of 2x + 3",key = f"user_input_{st.session_state.input_key}")
        ...
        if submitted and ui.strip():
            ...
            with st.spinner("????? Analyzing and solving your math problem..."):
                rsp = generate_responses(enhanced_prompt)
